...
Public Class frmTest
    ...
    Dim DifferentProperty As clsProperty
    Dim sp As PointF
    ...
    Private Sub picCanvas_MouseUp(ByVal sender As System.Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles picCanvas.MouseUp
        ...
        If Tool = ToolKind.Line Then
            Elems.Append(New clsLine(layers.Active, DifferentProperty, sp, ep))
            DifferentProperty = Nothing   '个性属性只供一个图形对象使用
        End If
        If Tool = ToolKind.Rectangle Then
            Elems.Append(New clsRectangle(layers.Active, DifferentProperty, sp, ep))
            DifferentProperty = Nothing   '个性属性只能供一个图形对象使用
        End If
    ...
    Private Sub m_Layer_Different_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles m_Layer_Different.Click
        Dim dlg As New ColorDialog
        If dlg.ShowDialog = vbOK Then
            '创建一个个性化的属性
            DifferentProperty = New clsProperty(New Pen(dlg.Color))
        End If
    End Sub
End Class
