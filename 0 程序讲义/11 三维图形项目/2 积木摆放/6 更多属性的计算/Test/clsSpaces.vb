...
Public Enum ArrangeMethod       '空间分配策略
    ' 将Remainment.delta作为剩余空间体积（其实是右上前角的体积）
    RemainVolumeMin     ' 剩余空间体积最小
    RemainVolumeMax     ' 剩余空间体积最大
End Enum

Public Class clsSpaces
    Private Spaces As List(Of clsBox)
    Public Shared Method As ArrangeMethod = ArrangeMethod.RemainVolumeMin
    ...
    Public Function Arrange(demand As clsBox) As Arrangement
        Dim gap As Vector3 = New Vector3(0.05, 0.05, 0.05)
        demand.Expand(gap)   '扩展demand一个缝隙
        Dim result As Arrangement
        '在Boxes中寻找与demand最合适的Boxes(index)
        Dim index As Integer = FindSuitableSpace(demand)
        If index = -1 Then
            demand.Expand(-gap)           ' 还原demand
            result.Flag = False
        ...
        demand.Move(box0.Base)   '将demand移到分配空间的坐下后角
        Spaces.Add(New clsBox(demand.BaseX, box0.Wx - demand.Wx, box0.Wy, box0.Wz))     ' ①
        Spaces.Add(New clsBox(demand.BaseY, demand.Wx, box0.Wy - demand.Wy, box0.Wz))   ' ②
        Spaces.Add(New clsBox(demand.BaseZ, demand.Wx, demand.Wy, box0.Wz - demand.Wz)) ' ③
        '2 分配Spaces(index)给demand
        result.Flag = True : result.Location = Spaces(index).Base + gap
        demand.Expand(-gap)           ' 还原demand
    ...
    Private Function Better(delta1 As Vector3, delta2 As Vector3) As Boolean
        ...
        If Method = ArrangeMethod.RemainVolumeMin Then
            Return v1 < v2
        End If
